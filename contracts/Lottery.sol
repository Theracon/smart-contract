...
error Lottery__UpkeepNotNeeded(
  uint256 loterryBalance,
  uint256 lotteryPlayers,
...
contract Lottery is VRFConsumerBaseV2, KeeperCompatibleInterface {
  ...
  event LotteryRecentWinners(address indexed recentWinner);
  ...
  /**
   * @dev This is the function that the Chainlink Keeper nbodes call,
   * they look for `upkeepNeeded   to return true.
   * They following should be true for upkeepNeeded to be true:
   * 1. The time interval should have passed
   * 2. The lottery should have at least 1 player and some ETH
   * 3. Our sucscription is funded with LINK
    ...
    )
  {
    ...
    upkeepNeeded = (intervalPassed && hasPlayers && hasBalance && isOpen);
  }
  ...
  function fulfillRandomWords(
    ...
    emit LotteryRecentWinners(recentWinner);
  }
